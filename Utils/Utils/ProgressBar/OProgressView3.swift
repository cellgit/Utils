...
@IBDesignable class OProgressView3: UIView {
    ...
    var dot = UIView()
    //头部箭头图片
    var arrow = UIImageView(image: UIImage(named: "audio_slider_dot"))
...
//            startAngle: angleToRadian(-90), endAngle: angleToRadian(270), clockwise: false)
                    startAngle: angleToRadian(-90), endAngle: angleToRadian(-450), clockwise: false)
...
//        let endAngle = angleToRadian(360*Double(progress)/100 - 90)
        ...
        let clockWise = progress > oldProgress ? true : false//false : true
